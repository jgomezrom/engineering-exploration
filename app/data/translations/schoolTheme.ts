// School names themselves (Illinois, Purdue, etc.) aren't translated — same
// convention as school/course names in collegeCurricula.ts. "Default" is UI
// chrome, not a proper noun, so it is.
export const schoolThemeTranslations = {
  en: {
    heading: "Choose a Color Theme",
    intro: "Pick a look for the site. Purely cosmetic — it doesn't change any content.",
    defaultName: "Default",
    comingSoon: "More coming soon",
    skip: "Skip for now",
    changeTheme: "Change color theme",
    close: "Close",
  },
  es: {
    heading: "Elige un Tema de Color",
    intro: "Elige un estilo para el sitio. Es puramente decorativo — no cambia ningún contenido.",
    defaultName: "Predeterminado",
    comingSoon: "Más próximamente",
    skip: "Omitir por ahora",
    changeTheme: "Cambiar tema de color",
    close: "Cerrar",
  },
};
